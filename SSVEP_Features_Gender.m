function SSVEP_Features_Gender(name, EEG, block)
...
% put to 0 preferably !!!!!!!!!
Screen('Preference', 'SkipSyncTests', 0)            % put to 0 preferably !!!!!!!!!
% put to 0 preferably !!!!!!!!!
...
load C:\Users\dFiset\Documents\Simon\SSVEP_Bubbles\stimuli\cropped_ims.mat
load C:\Users\dFiset\Documents\Simon\SSVEP_Bubbles\stimuli\meanbackgr.mat
% % % % % % % % % % % % % % % % % % % %% % % % %
...
%     
%     LastPath = fullfile(resultsDir,sprintf('%s_SSVEPGender_%d.mat',name,block-1));
...
cd 'C:\Users\dFiset\Documents\Simon\SSVEP_Expressions_Gender\Resultats';
...
ind_male_neutral=find(stim(:,3)==1&mod(stim(:,1),2)==0);
ind_male_happy=find(stim(:,3)==1&mod(stim(:,1),2)==1);
ind_female_neutral=find(stim(:,3)==2&mod(stim(:,1),2)==0);
ind_female_happy=find(stim(:,3)==2&mod(stim(:,1),2)==1);

xpstim_female=[ind_female_neutral; ind_female_happy];
xpstim_female=xpstim_female(randperm(length(xpstim_female)));
xpstim_male=[ind_male_neutral; ind_male_happy];
xpstim_male=xpstim_male(randperm(length(xpstim_male)));
...
for trial = 1:nb_trialsTot
    ...
    if mod(trial,1)==0 && trial~=1
        ...
        Screen('DrawText', windowPtr, sprintf('accuracy : %.3f, noise:%.2f', mean(acc_lastblock),1), xx_center+100, yy_center+100, [0,0,0,255]);
        Screen('Flip', windowPtr);
        KbWait;
    ...
    if EEG==1 && trigRespCounter~=1 && GetSecs-RespSecs>data{trial}.max_ISI
        ...
        % %  TRIGGER for new Face (== 100)
        outp(adress,TriggNewFace)
        WaitSecs(0.004);
        outp(adress,0)
...
%             Signal=QuestMean(Questy.q);
        ...
        if EEG==1 && trigRespCounter~=1 && GetSecs-RespSecs>data{trial}.max_ISI
            if data{trial}.Gender(GenderTrials) == 1 % which Gender is it (1==Man,0==woman)
                TriggWhichGender=251;
            else
                TriggWhichGender=250;
            end
            % %  TRIGGER for new Face (== 100)
            outp(adress,TriggNewFace)
            WaitSecs(0.004);
...
for trial = 1:nb_trialsTot
    data{trial}.Flipping(GenderTrials+1:end) = [];
    data{trial}.Gender(GenderTrials+1:end) = [];
end
