...
% visual angle.
% stimuli_width = 0.1; % to measure on screen in m, for instance
% width_deg = 1; % in deg
% viewing_distance = (stimuli_width/2)/tan((width_deg/2)/180*pi) % in m
...
StimuliPath  = fullfile(startPath,'Cropped_complete_images'); 
...
switch params.condition
    ...
    case 'gender' % right eye reinforced -- operant conditioning
...
%         viewing_distance_cm = 60;                                                                       % viewing distance in cm
...
% load('/Users/SimonFaghel-Soubeyrand/Documents/SSVEP_Bubbles/Stimuli_final/Stimuli_FEARGaborWavelet.mat','ellipse');
...
for trial=1:nTrials
    ...
    destRect = [round(rect(3)/2)-(params.ImSize/2) round(rect(4)/2)-(params.ImSize/2) round(rect(3)/2)+(params.ImSize/2) round(rect(4)/2)+(params.ImSize/2)]; % makes image 497 for the moment
...
accuracies(1,trial) = cid.data(9,trial);

s_accuracies = conv2(accuracies, ones(1,20)/20, 'valid');
